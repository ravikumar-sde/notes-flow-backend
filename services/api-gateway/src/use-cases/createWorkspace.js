const Joi = require('joi');

const schema = Joi.object({
  userId: Joi.alternatives(Joi.string(), Joi.number()).required(),
  name: Joi.string().trim().min(1).required(),
});

function createCreateWorkspaceUseCase({ workspaceApi }) {
  if (!workspaceApi) {
    throw new Error('workspaceApi is required to create createWorkspace use-case');
  }

  return async function createWorkspace({ userId, name }) {
    const { error, value } = schema.validate(
      { userId, name },
      { abortEarly: false, stripUnknown: true }
    );

    if (error) {
      const isMissingUserId = error.details.some(
        (d) => d.path[0] === 'userId' && d.type === 'any.required'
      );

      return {
        status: isMissingUserId ? 401 : 400,
        data: {
          message: 'Validation failed',
          details: error.details.map((d) => d.message),
        },
      };
    }

    try {
      const response = await workspaceApi.createWorkspace(
        { name: value.name },
        { userId: value.userId }
      );
      return { status: response.status, data: response.data };
    } catch (err) {
      if (err.response) {
        return { status: err.response.status, data: err.response.data };
      }
      console.error(
        'Error calling workspace service POST /workspaces',
        err.message
      );
      return {
        status: 502,
        data: {
          message: 'Workspace service unavailable',
          detail: err.message,
        },
      };
    }
  };
}

module.exports = { createCreateWorkspaceUseCase };

